...
<!-- ENFORCE: id=R10, hook=tool_call, tool=bash, match=vibe(?:\.py)?\s+evidence.*verify passed, action=check_bug_evidence, message=bug fix 必须有 reproduction + fix-regression 双向证据 -->
...
<!-- ENFORCE: id=R22, hook=tool_call, tool=bash, match=vibe(?:\.py)?\s+next, action=check_stage_transition, message=advance 前必须检查 gate 条件 -->
...
<!-- ENFORCE: id=R47, hook=tool_call, tool=bash, match=vibe amend-spec, action=check_prompt_version, message=amend 后必须 bump Prompt version，不能停留在旧版本 -->
...
<!-- ENFORCE: id=R62, hook=tool_call, tool=bash, match=vibe(?:\.py)?\s+commit.*reviewed, action=check_call_sites, message=commit 前必须 grep 所有受影响调用点，不能口头声称已检查 -->
...
<!-- ENFORCE: id=R66, hook=before_agent_start, action=inject_prompt, message=会话恢复后必须先运行 vibe status + vibe next -->

<!-- ENFORCE: id=R67, hook=tool_call, tool=bash, match=vibe(?:\.py)?\s+advance.*override_approver, action=check_override_session, message=override_approver must use non-current session actor, self-override forbidden -->
