...
def _classify_uncommitted_files(project_root: str) -> dict:
    ...
    NOTE: plan documents (.agents/plans/*.md) are intentionally NOT
    split out here — plans are auto-regenerated by `vibe plan`, so a
    dirty plan file is usually a stale artifact. The plan-regeneration
    flow already handles this; if we ever want to surface "plan is
    stale, regenerate" as a separate advisory, see TODO below.

    TODO: also handle .agents/plans/*.md (stale-plan advisory).
    """
    if not os.path.isdir(os.path.join(project_root, ".agents")):
